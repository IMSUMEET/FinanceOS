...
function EmphasisStat({ emoji, title, value, exact, sub, variant = "neutral" }) {
  ...
  return (
    <Card className={`${border} min-w-0 border-2`} padding="md" hover>
      ...
      <p
        className={`mt-2 tabular text-xl font-black tracking-tight sm:text-2xl ${valueTone}`}
        title={exact}
      >
        {value}
      </p>
      {sub ? (
        <p className="mt-2 text-xs leading-snug text-ink-500 dark:text-ink-400">{sub}</p>
      ) : null}
    </Card>
  );
}
...
function StatTile({ label, value, sub, exact }) {
  return (
    <div className="min-w-0 rounded-xl2 border border-white/60 bg-white/60 px-4 py-3 dark:border-ink-800 dark:bg-ink-900/50">
      ...
      <p
        className="mt-1.5 tabular text-base font-black tracking-tight text-ink-900 dark:text-ink-50 sm:text-lg"
        title={exact}
      >
        {value}
      </p>
      {sub ? (
        <p className="mt-1 text-[11px] leading-snug text-ink-500 dark:text-ink-500">{sub}</p>
      ) : null}
    </div>
  );
}
...
function ColorStatTile({ label, value, exact, colorClass }) {
  return (
    <div className={`min-w-0 rounded-xl2 border px-4 py-3 ${colorClass}`}>
      ...
      <p className="mt-1 tabular text-base font-black tracking-tight sm:text-lg" title={exact}>
        {value}
      </p>
    </div>
  );
...
function FlowDivider({ label }) {
  return (
    <div className="flex items-center gap-3 py-2">
      ...
      <span className="shrink-0 text-[11px] font-black uppercase tracking-wide text-ink-500 dark:text-ink-400">
        {label}
      </span>
      <div className="h-px flex-1 bg-ink-200/90 dark:bg-ink-700" />
    </div>
  ...
  return (
    <div className="min-w-0 space-y-1">
      <div className="flex min-w-0 items-start justify-between gap-2 text-xs">
        <span className="min-w-0 shrink font-semibold leading-snug text-ink-600 dark:text-ink-300">
          {label}
        </span>
...
function SellingOutcomeWaterfall({
  sale,
  commission,
  closing,
  payoff,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl2 border-2 border-brand-300 bg-brand-100 px-4 py-3 dark:border-brand-600 dark:bg-brand-900/60">
        <p className="text-xs font-bold uppercase tracking-wide text-brand-900 dark:text-brand-100">
          Cash at closing
        </p>
        <p
          className="mt-1 tabular text-lg font-black tracking-tight text-brand-950 dark:text-white"
          title={fmtWhole(cashAtClose)}
        >
          {fmtCompact(cashAtClose)}
        </p>
      </div>
      <WaterfallBar
        label="Money you put into the home (our count)"
        display={`−${fmtCompact(invested)}`}
      ...
      <div
        className={`rounded-xl2 border-2 px-4 py-3 ${trueProfit >= 0 ? "border-emerald-300 bg-emerald-100 dark:border-emerald-600 dark:bg-emerald-900/60" : "border-rose-300 bg-rose-100 dark:border-rose-600 dark:bg-rose-900/50"}`}
      >
        <p
          className={`text-xs font-bold uppercase tracking-wide ${trueProfit >= 0 ? "text-emerald-900 dark:text-emerald-100" : "text-rose-900 dark:text-rose-100"}`}
        >
          True profit / loss
        </p>
        <p
          className={`mt-1 tabular text-lg font-black tracking-tight ${trueProfit >= 0 ? "text-emerald-950 dark:text-white" : "text-rose-950 dark:text-white"}`}
        >
...
function HouseSaleCalculator() {
  ...
  const loanDonutData = useMemo(
    () =>
      buildLoanCompositionDonutData(
        loanSoFar.principalPaid,
        loanSoFar.interestPaid,
  ...
  const setTargetProfitFromSlider = useCallback((rawValue) => {
    setInput((prev) => ({
      ...prev,
      targetProfit: snapTargetProfitSlider(Number(rawValue), HOUSE_SALE_TARGET_PROFIT_SLIDER),
    }));
  }, []);
  ...
  return (
    <section className="space-y-8 pb-6 pt-2">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-2 max-w-2xl text-sm text-white/80">
              Three snapshots: what the loan has cost so far, what a sale does for your wallet, and
              what sale price hits your goal.
            </p>
          </div>
      ...
      {result.errors.length ? (
        <Card
          className="border-rose-200 bg-rose-50/80 dark:border-rose-900/40 dark:bg-rose-950/30"
          padding="md"
        >
            ...
            <div className="grid gap-6 lg:grid-cols-[minmax(0,200px)_1fr] lg:items-start">
              <div className="flex flex-col items-center">
                ...
                mortgage.remainingBalance > 0 ? (
                  <>
                    ...
                    />
                    <p className="mt-2 max-w-[12rem] text-center text-[11px] text-ink-500 dark:text-ink-500">
                      Hover slices to compare.
                    </p>
                  </>
                ) : (
                  <p className="max-w-[12rem] text-center text-sm text-ink-500 dark:text-ink-400">
                    Fill in purchase details to see your loan breakdown.
                  </p>
                )}
              </div>
              <div className="grid min-w-0 gap-3 sm:grid-cols-2">
                <StatTile
                  ...
                  value={fmtCompact(mortgage.originalLoanAmount)}
                  sub="Price − down"
                />
                <StatTile
                  label="Monthly payment"
                  value={fmtWhole(mortgage.monthlyPayment)}
                  sub="Principal & interest"
                />
                <StatTile
                  label="Months paid"
                  value={String(loanSoFar.monthsPaid)}
                  sub="Since purchase"
                />
                <StatTile
                  label="Total paid so far"
                  value={fmtCompact(loanSoFar.totalPaid)}
                  sub="Payment × months"
                />
                <ColorStatTile
                  label="Equity built (principal)"
                  value={fmtCompact(loanSoFar.principalPaid)}
                  exact={fmtWhole(loanSoFar.principalPaid)}
                  colorClass="border-emerald-200/90 bg-emerald-50/70 text-emerald-900 dark:border-emerald-800/50 dark:bg-emerald-950/25 dark:text-emerald-100"
                />
                <ColorStatTile
                  label="Interest cost"
                  value={fmtCompact(loanSoFar.interestPaid)}
                  exact={fmtWhole(loanSoFar.interestPaid)}
                  colorClass="border-amber-200/90 bg-amber-50/80 text-amber-950 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-100"
                />
                <div className="sm:col-span-2">
                  <ColorStatTile
                    label="Still owed"
                    value={fmtCompact(mortgage.remainingBalance)}
                    exact={fmtWhole(mortgage.remainingBalance)}
                    colorClass="border-ink-200/80 bg-white/80 text-ink-900 dark:border-ink-700 dark:bg-ink-900/60 dark:text-ink-50"
                  />
                </div>
              </div>
            </div>
        ...
        <div className="space-y-5">
          <InputSection title="Selling assumptions" icon={TrendingUp} defaultOpen>
            ...
            <p className="mt-3 text-xs text-ink-500 dark:text-ink-400">
              ...
              {HOUSE_SALE_RATE_DEFAULTS.closingCostsPct}% when blank.
            </p>
          </InputSection>
        ...
        >
          <Card padding="lg" className="border-white/60 dark:border-ink-800">
            <SectionHeading
              ...
              description="What your expected price means for money in your pocket."
            />
            <div className="mb-5 grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 [&>*]:min-w-0">
              <EmphasisStat
                emoji="💵"
                title="Cash at closing"
                value={fmtCompact(result.netProceeds)}
                exact={fmtWhole(result.netProceeds)}
              ...
              <EmphasisStat
                ...
                value={fmtCompact(result.trueProfit)}
                exact={formatCurrency(result.trueProfit, { signed: true })}
              ...
              <EmphasisStat
                ...
                value={
                  result.breakEvenSalePrice == null ? "—" : fmtCompact(result.breakEvenSalePrice)
                }
                exact={
                  result.breakEvenSalePrice == null
                    ? undefined
                    : fmtWhole(result.breakEvenSalePrice)
                }
                sub="After fees, payoff, and your counted costs"
                variant={breakEvenCardVariant}
              />
              <EmphasisStat
                emoji="🎯"
                title="Price for target profit"
                value={result.targetSalePrice == null ? "—" : fmtCompact(result.targetSalePrice)}
                exact={
                  result.targetSalePrice == null ? undefined : fmtWhole(result.targetSalePrice)
                }
                sub={`For about ${fmtCompact(result.targetProfitInput)} true profit`}
                variant="neutral"
              />
            </div>
            <div className="rounded-xl2 border border-brand-200/70 bg-brand-50/60 px-4 py-3 dark:border-brand-800/50 dark:bg-brand-950/25">
              ...
              <p className="mt-1 text-sm text-ink-800 dark:text-ink-200">
                ...
                is what's left after the sale costs and loan payoff.{" "}
                <span className="font-bold text-brand-800 dark:text-brand-200">True profit</span>{" "}
                also backs out all invested cash.
              </p>
            </div>
          </Card>
        </Motion.div>
      ...
      <div className="grid grid-cols-1 gap-8 xl:grid-cols-2 xl:items-start">
        <div className="space-y-5">
          <InputSection title="Target profit" icon={Target} defaultOpen>
            ...
            <div className="mt-4 space-y-2">
              <div className="flex items-center justify-between text-[11px] font-bold uppercase tracking-wide text-ink-500 dark:text-ink-400">
                <span>Quick set</span>
                <span className="tabular text-ink-700 dark:text-ink-200">
                  {formatCurrency(sliderSnap)}
                </span>
              </div>
              <input
                type="range"
          ...
          <Card padding="lg" className="border-white/60 dark:border-ink-800">
            ...
            />
            <Motion.div
              className="mt-2 min-h-[280px] min-w-0 w-full rounded-xl2 border border-white/60 bg-white/50 p-3 dark:border-ink-800 dark:bg-ink-900/40"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
      ...
      <div className="grid grid-cols-1 gap-8 xl:grid-cols-2 xl:items-start">
        ...
        >
          <Card padding="lg" className="border-white/60 dark:border-ink-800">
            <SectionHeading
              ...
              description="Guardrails—not a title-company quote."
            />
            <div className="grid min-w-0 grid-cols-1 gap-6 sm:grid-cols-2 [&>*]:min-w-0">
              <div>
                ...
                <p
                  className="mt-1 tabular text-xl font-black tracking-tight text-ink-900 dark:text-ink-50 sm:text-2xl"
                  title={
                    result.breakEvenSalePrice == null
                      ? undefined
                      : fmtWhole(result.breakEvenSalePrice)
                ...
                <p className="mt-2 text-sm text-ink-600 dark:text-ink-400">
                  Roughly flat after fees, payoff, and invested cash.
                </p>
              </div>
              <div>
                <p className="text-xs font-bold uppercase tracking-wide text-ink-500 dark:text-ink-400">
                ...
                >
                  {result.targetSalePrice == null ? "—" : fmtCompact(result.targetSalePrice)}
                </p>
                <p className="mt-2 text-sm text-ink-600 dark:text-ink-400">
                  The price that leaves about{" "}
                  <span className="font-bold text-ink-800 dark:text-ink-200">
                    {fmtCompact(result.targetProfitInput)}
                  </span>{" "}
                  of true profit.
                </p>
              </div>
              {result.minSalePriceNetProceedsZero != null ? (
                <p className="text-xs text-ink-500 dark:text-ink-500 sm:col-span-2">
                  Just need $0 at closing? That's about{" "}
                  <span className="font-bold text-ink-700 dark:text-ink-300">
                    {fmtCompact(result.minSalePriceNetProceedsZero)}
                  </span>
                  —not the same as truly breaking even.
                </p>
              ) : null}
            </div>
          </Card>
        ...
        <div className="space-y-6">
          {narrative.length ? (
            <Card
              className="border-amber-200/80 bg-amber-50/50 dark:border-amber-900/35 dark:bg-amber-950/20"
              padding="md"
            >
