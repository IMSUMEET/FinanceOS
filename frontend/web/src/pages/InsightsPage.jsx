...
function InsightsPage() {
  ...
  const isAiTab = pathname === "/insights/ai";
  useDocumentTitle(isAiTab ? "AI Insights" : "Insights");
  ...
  return (
    <section className="space-y-5 pt-2">
      ...
      {isAiTab ? (
        ...
        <>
          <Card className="rounded-xl3 border border-border-subtle bg-white text-ink-900 shadow-soft dark:border-white/10 dark:bg-gradient-to-br dark:from-[#0d1527] dark:via-[#09101d] dark:to-[#070b14] dark:text-white dark:shadow-dark">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-brand-600 dark:text-brand-300">
                  Insight Engine
                </p>
                <h2 className="mt-2 text-2xl font-black md:text-3xl text-ink-900 dark:text-white">
                  {mom.deltaPct == null
                    ? "Build a baseline"
                    : mom.deltaAbs >= 0
                ...
                <p className="mt-2 text-ink-600 max-w-2xl text-sm leading-relaxed dark:text-slate-300">
                  {momCopy}
                </p>
              </div>
              {mom.deltaAbs >= 0 ? (
                <TrendingUp className="text-rose-500 dark:text-rose-400 shrink-0" size={36} />
              ) : (
                <TrendingDown
                ...
                {recurring.length === 0 ? (
                  ...
                  recurring.slice(0, 5).map((r) => (
                    ...
                    >
                      ...
                      <Badge tone="brand">
                        {formatCurrency(r.annualized, { compact: true })}/yr
                      </Badge>
                    </div>
                  ))
                )}
