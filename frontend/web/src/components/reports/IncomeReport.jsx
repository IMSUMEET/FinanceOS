...
function IncomeReport({ transactions }) {
  const data = useMemo(() => {
    ...
    for (const t of incomeTx) {
      ...
      if (desc.includes("salary") || desc.includes("paycheck") || desc.includes("deposit") || desc.includes("payroll")) {
      ...
      } else if (desc.includes("freelance") || desc.includes("gig") || desc.includes("stripe") || desc.includes("consult")) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center border-b border-ink-100 pb-3 dark:border-ink-800">
        <div>
          <h3 className="text-md font-bold text-ink-900 dark:text-ink-50">Income Source Distribution</h3>
      ...
      <div className="grid gap-6 md:grid-cols-2 items-center">
        ...
        <div className="space-y-3">
          {data.map((item, idx) => (
            <div key={item.name} className="flex justify-between items-center text-sm border-b border-ink-50 pb-2 dark:border-ink-900/50">
              <span className="font-semibold text-ink-850 dark:text-ink-200">{item.name}</span>
              <div className="text-right font-mono">
                <span className="font-bold text-ink-900 dark:text-ink-50">${item.value.toLocaleString()}</span>
