// 10 Canonical Categories Engine
const RULES = [
  [
...
export function mapCanonicalCategory(catName) {
...
export function normalizeMerchant(raw) {
  ...
  // Special distinction preserved: Costco Gas vs Costco Warehouse
  const isCostcoGas = /costco\s*gas/i.test(s);
  if (isCostcoGas) return "Costco Gas";

  // Clean common noise: store numbers, phone numbers, state codes, transaction IDs, trailing asterisks
  s = s
    .replace(/(#|\bstore\b|\bno\.?|\bloc\b|\bunit\b)\s*\d+/gi, "")
    .replace(/\b(online|www\.|http|\.com|sq\*|tst\*|sp\*|paypal\*)\b/gi, "")
    .replace(/\b[A-Z]{2}\s+\d{5}(-\d{4})?\b/gi, "") // Zip codes
    .replace(/\d{4,}/g, "")
    .replace(/\*[A-Z0-9]+/gi, "")
    .replace(/[*#@]/g, " ")
    .replace(/\s+/g, " ")
